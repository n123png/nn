...
│                     (主控调度器)                          │
├──────────┬──────────┬──────────┬──────────┬──────────────┤
│ sensors  │  npc_    │ hazard_  │  kpi_    │   ui_        │
│ (传感器) │ manager  │ manager  │evaluator │ dashboard    │
│          │(NPC管理) │(危险场景)│(KPI评估) │ (仪表盘)     │
├──────────┼──────────┼──────────┼──────────┼──────────────┤
│ map_     │ traffic_ │ blackbox │ recorder │   player     │
│ drawer   │ light_   │(黑匣子)  │ (录制)   │  (回放)      │
│(鸟瞰图)  │ monitor  │          │          │              │
│          │(红绿灯)  │          │          │              │
└──────────┴──────────┴──────────┴──────────┴──────────────┘
           │               │               │
           ▼               ▼               ▼
    ┌─────────────┐ ┌────────────┐ ┌──────────────┐
    │  CARLA API   │ │ 传感器数据  │ │ kpi_report   │
    │  (仿真引擎)  │ │  (实时流)   │ │ blackbox.csv │
    └─────────────┘ └────────────┘ └──────────────┘
```
